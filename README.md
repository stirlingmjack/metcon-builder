...
- **`js/strength-data.js`** — the strength program: a list of days, each a
  list of lettered blocks (`A`, `B1`/`B2` superset, ..., a `D` finisher).
  A block is either a logged lift (`sets`/`reps`) or a `finisher`
  (`{format, durationMinutes}`, generated by the Metcon engine). Add more
  days here — the day picker and everything else is driven by this data.
- **`js/strength.js`** — wires the Strength tab to the DOM: renders the
  day's blocks (grouping supersets), logs weight/reps per set, generates
  the finisher via `MetconGenerator` using the same equipment/space
  Settings as the Metcon tab (refreshing it if you change equipment
  mid-session), and keeps its own history. Self-contained — only depends
  on the already-loaded globals, same as `app.js`.
